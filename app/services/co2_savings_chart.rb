class Co2SavingsChart < Flux::Reader
    CO2_EMISION_FACTOR = 401.0 # g / kWh
    ...
    def chart_single(start:, window:, stop: nil, fill: false)
      ...
      q << "|> map(fn: (r) => ({ r with co2_savings: r.#{inverter_power_field} / 1000.0 * #{CO2_EMISION_FACTOR} / 1000.0}))"
      q << '|> keep(columns: ["_time", "co2_savings"])'
    ...
    def chart_sum(start:, window:, stop: nil)
      raw = query <<~QUERY
        ...
        |> map(fn: (r) => ({ r with co2_savings: r.#{inverter_power_field} / 1000.0 * #{CO2_EMISION_FACTOR} / 1000.0 }))
        |> keep(columns: ["_time", "co2_savings"])
      QUERY
    ...
    def value_to_array(raw)
      ...
      raw&.records&.each_with_index do |record, index|
        ...
        value = next_record.values['co2_savings']
