class Calculator::Range < Calculator::Base # rubocop:disable Metrics/ClassLength
  ...
  def build_context(data)
    ...
    build_method_from_array(:house_power_grid, data, :to_i)
    build_method_from_array(:wallbox_power_grid, data, :to_i)
    build_method_from_array(:heatpump_power_grid, data, :to_i)
  ...
  def total_power_grid
    wallbox_power_grid + heatpump_power_grid + house_power_grid
  end

  def total_power_grid_ratio
    total_power.zero? ? 0 : (total_power_grid.fdiv(total_power) * 100).round
  end
  ...
  def wallbox_power_grid_ratio
    if wallbox_power.zero?
      ...
      (wallbox_power_grid.fdiv(wallbox_power) * 100).round
    end
  end
  ...
  def heatpump_power_grid_ratio
    if heatpump_power.zero?
      ...
      (heatpump_power_grid.fdiv(heatpump_power) * 100).round
    end
  end
  ...
  def house_power_grid_ratio
    house_power.zero? ? 0 : (house_power_grid.fdiv(house_power) * 100).round
  end
