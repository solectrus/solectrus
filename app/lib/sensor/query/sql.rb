module Sensor
  module Query
    ...
    #   query = Sensor::Query::Sql.new do |q|
    #     q.sum :house_power
    #
    #     q.timeframe Timeframe.day
    #   end
    #   data = query.call
    ...
    class Sql < Base
      def initialize(&block)
        ...
        super(sensor_names, dsl_config[:timeframe])

        # Store configuration
        @original_sensor_requests = dsl_config[:requests]
        @timeframe = dsl_config[:timeframe]
      ...
      def build_dsl_configuration(&)
        builder = Helpers::SqlDslBuilder.new
        yield(builder)

        {
          requests: builder.sensor_requests,
          timeframe: builder.timeframe_value,
      ...
      def find_finance_dependencies
        @original_sensor_requests.each_with_object(
          ...
          if sensor.sql_calculated? || (sensor.calculated? && sensor.respond_to?(:sql_calculation))
      ...
      def finance_dependency_not_requested?(sensor_name, finance_dependencies)
      ...
      def find_requesting_sensors(sensor_name)
        sensors = @original_sensor_requests.map { |req| Sensor::Registry[req.first] }
      ...
      def format_query_result(raw_result)
        ...
        mapper =
          Helpers::SqlResultMapper.new(sensor_requests, group_by:, timeframe:)
