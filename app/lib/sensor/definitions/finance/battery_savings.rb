class Sensor::Definitions::BatterySavings < Sensor::Definitions::FinanceBase
  depends_on :battery_discharging_power, :battery_charging_power

  def required_prices
    %i[electricity feed_in]
  end

  def sql_calculation
    # Savings from battery discharge (what you save by not buying from grid)
    # minus lost revenue from charging (what you lose by not feeding into grid)
    '(COALESCE(battery_discharging_power_sum, 0) * pb_money_per_kwh - COALESCE(battery_charging_power_sum, 0) * pf_money_per_kwh) / 1000.0'
  end

  def calculate_with_prices(battery_discharging_power:, battery_charging_power:, prices:)
    electricity_price = prices[:electricity]
    feed_in_price = prices[:feed_in]
    return unless electricity_price && feed_in_price

    discharge = battery_discharging_power || 0
    charge = battery_charging_power || 0

    # Savings = value of discharged power - opportunity cost of charged power
    ((discharge * electricity_price) - (charge * feed_in_price)) / 1000.0
  end
end
