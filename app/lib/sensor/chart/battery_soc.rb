class Sensor::Chart::BatterySoc < Sensor::Chart::Base
  ...
  def build_sql_series
    Sensor::Query::Sql
      .new do |q|
        q.avg :battery_soc, :min
        q.avg :battery_soc, :max
  ...
  def build_data
    ...
    min_points = series.battery_soc(:avg, :min)
    max_points = series.battery_soc(:avg, :max)
    ...
    sensor = Sensor::Registry[:battery_soc]
    {
      labels:,
